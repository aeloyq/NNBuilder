...
class layer():
    ...
    def evaulate(self):
        if isinstance(self.input,layers.baselayer):
            ...
            inp = self.input
        self.layer.build(inp,self.name)
...
class node:
    ...
    def evaulate(self):
        ...
        if self.operation == '+':
            ...
            self.output = T.concatenate([self.id1, self.id2, 1])
from layers.layers import ops
class dropout:
    def __init__(self, layer, dp_name=None, use_noise=0.5):
        self.layer = layer
        if dp_name != None and use_noise
    ...
    def evaulate(self):
        if self.dp_name==None:
            self.op_dict={ops.dropout.name:{ops.dropout.use_noise:self.use_noise}}
        else:
            self.op_dict = {ops.dropout.name: {ops.dropout.use_noise: self.use_noise}}
            ...
            for name in self.dp_name:
                ...
                self.op_dict [name]={ops.dropout.use_noise: self.use_noise}
