...
import numpy as np
import lstm, recurrent

baselayer_lstm = lstm.get
baselayer_rnn = recurrent.get
...
class get_lstm(baselayer_lstm):
    ...
    def get_state_before(self):
        inp = self.input_way(self.input, self.x_mask)
...
class get_lstm_attention_maxout_readout_feedback(baselayer_lstm):
    def __init__(self, in_dim, unit_dim, emb_dim, h_0_init=False, c_0_init=False, r_0_init=False, activation=T.tanh,
                 **kwargs):
        baselayer_lstm.__init__(self, in_dim, unit_dim, emb_dim, h_0_init, c_0_init, activation, **kwargs)
        ...
        self.wt_feedback = 'wt_feedback'
        self.bi_attention = 'bi_attention'
        self.bi_readout = 'bi_readout'
        self.bi_feedback = 'bi_feedback'
        self.params.append(self.wt_attention, self.wt_readout, self.wt_feedback, self.bi_attention, self.bi_readout,
                           self.bi_feedback)

        def init_layer_params(self):
            baselayer_lstm.init_layer_params(self)
            if self.r_0_init:
                r_0_values = np.zeros(self.unit_dim, 'float32')
