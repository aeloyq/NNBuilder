...
class attention(layer):
    ...
    def apply(self, X, P):
        ...
        if self.o_dim == 1:
            ...
            att = att_layer_2
            if att.ndim == 4:
                eij = T.exp(att-att.max(0,keepdims=True)) * mask[:, :, None, None]
            else:
                eij = T.exp(att-att.max(0,keepdims=True)) * mask[:, :, None]
            aij = eij / eij.sum(0, keepdims=True)
            return aij
...
class decoder(sequential):
    def __init__(self, unit, emb_dim, vocab_size, core=gru,
                 state_initiate='mean', in_channel=2, is_maxout=True, is_ma=False, **kwargs):
        sequential.__init__(self, **kwargs)
        self.unit_dim = unit
        self.emb_dim = emb_dim
    ...
    def prepare(self, X, P):
        ...
        mean_ctx = (self.input[:, :, -self.in_dim//self.in_channel:] * self.x_mask[:, :, None]).sum(0) / self.x_mask.sum(0)[:, None]
        s_0 = mean_ctx
    ...
    def gen_sample_(self, beam_size=12):
        ...
        initiate_state.extend([y_mm_0, T.constant(0), None, None, None, None,None])
        ...
        def step_gru(y_emb_, s_, y_mm_b_k_, idx_1_, pctx, ctx, x_m, swt, sbi, swtg, sbig, su, sug, adwt,
                     acwt, acbi, gwt,
                     gbi, gwtg,
                     gbig, gu, gug, erwt, erbi, epwt, egwt, edwt, wemb):
            '''
            ...
            prob_bk_v = -T.log(prob_bk_v * y_mm_b_k_.flatten()[:, None])
            ...
            y_trans=(y_b_k // self.vocab_size).flatten()+T.repeat(T.arange(self.n_samples), self.beam_size)*self.beam_size
            s=T.reshape(s.reshape( [self.n_samples*self.beam_size, self.unit_dim])[T.cast(y_trans, 'int64')], [self.n_samples, self.beam_size, self.unit_dim])
            ...
            return [y_emb, s, y_mm, idx_1, prob_b_k, y_mm_shifted_b_k, y_b_k, y_mod,y_trans], theano.scan_module.until(T.eq(y_mm.sum(), 0))
        ...
        if self.core == lstm:
            ...
            self.y_emb, self.s, self.y_mm, self.idx, self.prob, self.y_mm_shifted, self.y_pred, self.y_mod,self.s_c = result

        def dec(mm,ms, y_pred, y_mod,y_prob, y_idx_):
            def d(mm,ms,y_idx_,pb):
                msm=T.xor(mm,ms)
                lc = mm.sum()
                ls = msm.sum()
                eid=T.argsort(msm)[::-1]
                ...
                score=pb[y_idx]
                score=T.set_subtensor(score[lc + ls:],0)
                return y_idx,score
            [y_idx_c,score],u=theano.scan(d,[mm,ms,y_idx_,y_prob])
        ...
        [self.y_idx, self.y_out,self.score,self.y_idx_c], updates = theano.scan(dec, sequences=[self.y_mm,self.y_mm_shifted, self.y_pred, self.y_mod,self.prob],
                                                    outputs_info=[idx_list_0, None,None,None], go_backwards=True)
        self.sample_updates = OrderedDict()
        self.sample_updates.update(self.gen_updates)
        self.sample_updates.update(self.updates)
        self.score_sum=self.score.sum(0)/self.y_mm_shifted.sum(0)
    ...
    def gen_sample(self, beam_size=12):
        ...
        def step_gru(y_emb_, s_,prob_sum_b_k_, y_mm_b_k_, idx_1_, pctx, ctx, x_m, swt, sbi, swtg, sbig, su, sug, adwt,
            ...
            prob_b_k = prob_b_kv[
                ...
                [self.n_samples, self.beam_size])
            y_mm = y_mm_c_b_k * y_mm_shifted_b_k
            return [y_emb, s,prob_b_k, y_mm, idx_1,  y_mm_shifted_b_k, y_b_k, y_mod,y_trans], theano.scan_module.until(T.eq(y_mm.sum(), 0))
