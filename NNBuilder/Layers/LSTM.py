...
class layer(Hidden_Layer):
    def __init__(self, Rng, N_in, N_hl,out_state='final', Name='undefined', Wt=None, Bi=None, U=None, Wt_init='uniform',
    ...
    def init_wt_bi(self):
        if not self.wt_bi_inited:
            Wt_values, Bi_values = Layer_Tools.Fully_connected_lstm_weights_init(self.Rng, self.N_in, self.N_units,
    ...
    def output_func(self):
        ...
        lin_out=lin_out[0]
        if self.out_state=='final':
            self.outputs = lin_out[-1]
        elif self.out_state=='all':
            lin_out = lin_out * self.mask[:, :, None]
            self.outputs = lin_out
        elif self.out_state == 'mean_pooling':
            lin_out = lin_out * self.mask[:, :, None]
            self.outputs = T.mean(lin_out,axis=0)
        self.add_debug(self.outputs)
        self.add_debug(lin_out)